class ServiceManager {
    ...
    showSystemAndServiceStatus() {
        return showDialog({
            ...
            size: 'lg',
            onShow: () => {
                this.updateAllStatus();  // 使用 this 调用类方法
            }
        });
    }
    ...
    async updateAllStatus() {
        try {
            ...
            // 更新服务状态
            this.updateServiceStatus('storeScp', data.store);
            this.updateServiceStatus('worklistScp', data.worklist);
            this.updateServiceStatus('qrScp', data.qr);
            this.updateServiceStatus('printScp', data.print);
